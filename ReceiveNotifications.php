     ...
     */
	 ...
	 if(isSuccessfulPayment($transactionType)) {
		 ...
		 if($memo && strpos($memo, 'Membership') !== 0 && $paymentAmount < 400) {
		        $googleDrive->updateRow($senderEmail, "month");
		 }
		 else if($memo && strpos($memo, 'Membership') !== 0 && $paymentAmount >= 400) {
		        $googleDrive->updateRow($senderEmail, "year");
		 }
		 ...
		 if($paymentAmount == -40 || $paymentAmount == -37.50 || $paymentAmount == 60) {
			 $googleDrive->updateRow($senderEmail, "removeMonth");
		 }
		 if($paymentAmount == -430 || $paymentAmount == -400 || $paymentAmount == -645) {
			 $googleDrive->updateRow($senderEmail, "removeYear");
		 }
...
function isSuccessfulPayment($transactionType) {
	if($transactionType === "web_accept"
		...
		|| $transactionType === "merch_pmt"
		|| $transactionType === "pro_hosted"
		|| $transactionType === "recurring_payment"
		|| $transactionType === "subscr_payment"
		|| $transactionType === "send_money"
		|| $transactionType === "virtual_terminal"
...
function isPaymentFailure($transactionType) {
	if($transactionType === "recurring_payment_expired"
		|| $transactionType === "recurring_payment_failed"
		|| $transactionType === "recurring_payment_suspended_due_to_max_failed_payment"
		|| $transactionType === "recurring_payment_suspended"
		|| $transactionType === "subscr_eot"
		|| $transactionType === "subscr_failed") {
			return true;
		}
	return false;
}
